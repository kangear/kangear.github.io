...
某用户摸索一个小时仍然搜索不到设备，使用`Discovery-DNS-SD Browser`App测试如下图所示，仅能搜索到局域网中`隔空投送`相关设备。上图3中对应的`hap`协议完全没有搜索到，其中HAP就是`HomeKit Accessory Protocol`的缩写，该协议基于`mDNS`底层协议。这就说明路由器禁用了该协议，通过另外一台iPhone开启热点对比测试，进一步印证了猜测，该用户的路由器为电信宽带赠送的`Tewa-1006G`型号。

| [<img src="/assets/9c76ebd5070ba50f71808d188466d83.jpg" width="300"/>](/assets/9c76ebd5070ba50f71808d188466d83.jpg)|
| :------------: |
|          搜索不到`hap`案例    |

对应的解决方案：不使用运营商提供的路由器，改用普通路由器。
