...
用户摸索一个小时仍然搜索不到设备，使用`Discovery-DNS-SD Browser`App测试如下图所示，仅能搜索到局域网中`隔空投送`相关设备，上图3中对应的`hap`协议，其中HAP就是`HomeKit Accessory Protocol`的缩写，该协议基于`mDNS`底层协议，说明路由器禁用了该协议；使用另外一台iPhone开启热点避开该路由器测试，可以正常搜索和添加，进一步证明了猜测。用户的路由器为电信宽带赠送的`Tewa-1006G`型号。
