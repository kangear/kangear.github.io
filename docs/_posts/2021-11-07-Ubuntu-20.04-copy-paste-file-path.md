...
```
这下效率就降低不少。下面是解决方案。**以下方案其实没有什么用，我快对20.04失去耐心了，最常用的桌面功能要改革，没有了右击新建文件，不支持放文件，图标还必须一个格子格子的摆放。系统本来都是用来作为生产工具的，搞这么多变数。老牌操作系统Win/Mac对这一点都没有改变，一个不知流的Linux桌面系统在这改来改去的。还好没有在去年底换20.04，一直使用着16.04和18.04。到时候看看`22.04`会怎么样吧**。
